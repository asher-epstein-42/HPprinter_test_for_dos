...
# function to filter frames by the first half of the destination mac address(that determine that this is a hp #device -> 9C:7B:EF) and  if arp opcode == 1 (to check if this is an arp request to one of the printers)
...
def stealth_scan(hp_device_ip, hp_device_port):
    # perform stealth scan on port 9100 to see if this is a printer (9100 used for raw printing)
...
# The purpose of this script is to check if someone is trying to connect to an HP printer over the network,
...
# for every hp device we check if this is a printer by performing a stealth scan using our captured destination IP address and port 9100 (raw printing) over tcp to see if we manage to get any response
